  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sticksAndMallets = data.items.filter(
          (drum: any) => drum.fields.category === "Sticks and Mallets"
        );

        setDrums(sticksAndMallets);
  ...
  return (
    <section className="flex flex-col justify-center items-center align-middle text-center text-base-colors-200 m-0">
      ...
      <div   className={`flex flex-row items-center w-[80vw] md:w-[50vw] font-roboto font-medium text-xs md:text-xl mx-0 md:mx-auto mt-1 md:mt-2 ${
        (currentPage > 1 && indexOfLastItem < drums.length) ? 'justify-between' : 'justify-center'
      }`}>
        {currentPage > 1 && (
...
export default FetchSticksAndMallets;