...
const Sort: FC<SortProps> = ({items, setItems, setCurrentPage}) => {
    ...
    return (
        <div className="font-roboto font-medium text-base lg:text-xl w-auto lg:w-screen flex flex-col lg:flex-row justify-center items-center space-x-0 lg:space-x-4 mt-4 mb-4 z-10">
        {/* Buttons for larger screens */}
            <div className="hidden lg:flex md:space-x-4">
            <button
                onClick={sortByRelevance}
                className={`px-4 py-2 bg-base-colors-200 text-base-colors-50 rounded-tl-3xl rounded-br-3xl hover:bg-base-colors-300 active:bg-base-colors-300 ${
            ...
            {isDropdownOpen && (
                <div ref={sortRef} className="absolute mt-12 bg-base-colors-200 text-base-colors-50 rounded-lg shadow-lg">
                    <button
                        onClick={sortByRelevance}
                        className={`flex px-4 py-2 active:bg-base-colors-300 w-full ${
                        sortOrder === "" ? "bg-base-colors-300" : ""
                        }`}
                    >
                        Relevance
